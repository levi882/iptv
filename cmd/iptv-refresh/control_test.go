...
func TestControlRequest(t *testing.T) {
	...
	body, err := controlRequest(server.Client(), server.URL, "status", "", "")
	if err != nil || string(body) != `{"ok":true}` {
		t.Fatalf("status response = %q, %v", body, err)
	}
	body, err = controlRequest(server.Client(), server.URL, "refresh", "secret", "eth3.3927")
...
func TestControlRequestRejectsUnknownActionAndHTTPError(t *testing.T) {
	t.Parallel()
	if _, err := controlRequest(nil, "http://127.0.0.1", "invalid", "", ""); err == nil {
	...
	t.Cleanup(server.Close)
	if _, err := controlRequest(server.Client(), server.URL, "playlist", "secret", ""); err == nil || !strings.Contains(err.Error(), "403") {
