...
PACKAGE=${1:-/tmp/luci-app-iptv-refresh-0.1.0-r7.apk}
I18N_PACKAGE=/tmp/luci-i18n-iptv-refresh-zh-cn-0.1.0-r7.apk
MODE=
case "${2:-}" in
...
apk info -e iptv-refresh >/dev/null 2>&1 || {
	echo "ERROR: install iptv-refresh-0.1.0-r6.apk first" >&2
	exit 1
}
