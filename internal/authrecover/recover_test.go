...
func TestRecoverNumericAllowsUnknownDeviceFields(t *testing.T) {
	...
	plain := "1$token$user$stb$10.0.0.3$AA:BB:CC:DD:EE:FF$Reserved$CTC"
	sample.Authenticator = encryptForTest(t, "1", []byte(plain))
	result, err := RecoverNumeric(context.Background(), sample, 1)
	if err != nil {
		t.Fatal(err)
	}
	if result.STBID != "stb" || result.IP != "10.0.0.3" || result.MAC != "AA:BB:CC:DD:EE:FF" || result.Reserved != "Reserved" {
		t.Fatalf("unexpected result: %#v", result)
	}
}
